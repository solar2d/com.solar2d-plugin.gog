...
class LuaMethodCallback
{
	public:
		...
		LuaMethodCallback()
		:	LuaMethodCallback(nullptr, nullptr, nullptr)
		{
		}

		/**
		  Creates and registers a Lua function which will invoke the given C++ object's method.
		  ...
		  Can be null, causing this callback to no-op and not register a function into Lua.
		 */
		LuaMethodCallback(TClass* objectPointer, MethodType methodPointer, lua_State* luaStatePointer)
		:	fObjectPointer(nullptr),
			fMethodPointer(nullptr),
			fLuaStatePointer(nullptr),
			fLuaRegistryReferenceId(LUA_NOREF)
		{
			// Validate.
			if (!objectPointer || !methodPointer || !luaStatePointer)
			{
				return;
			}
			...
			int referenceId = luaL_ref(luaStatePointer, LUA_REGISTRYINDEX);

			// Update member variables if successfully set up a Lua closure up above.
		...
		/**
		  ...
		  This can happen if this callback's Release() method was called or if the callback's constructor
		...
		void Release()
		{
			...
			if (fLuaStatePointer)
			{
				...
				lua_rawgeti(fLuaStatePointer, LUA_REGISTRYINDEX, fLuaRegistryReferenceId);
				lua_pushlightuserdata(fLuaStatePointer, nullptr);
			...
			// This makes this object's methods no-op.
			fObjectPointer = nullptr;
			fMethodPointer = nullptr;
			fLuaStatePointer = nullptr;
			fLuaRegistryReferenceId = LUA_NOREF;
	...
	private:
		...
		static int OnInvoked(lua_State* luaStatePointer)
		{
			...
			// Will be null if the callback's Release() method was called.
